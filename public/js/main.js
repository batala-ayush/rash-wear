...
function initNav(){
  ...
  const linksCollection = document.querySelectorAll(".nav-links");
  if (!toggle || !linksCollection.length) return;
  toggle.addEventListener("click", () => {
    const isOpen = !toggle.classList.contains("open");
    linksCollection.forEach(links => links.classList.toggle("open", isOpen));
    toggle.classList.toggle("open", isOpen);
  });
  linksCollection.forEach(links => {
    links.querySelectorAll("a").forEach(a => a.addEventListener("click", () => {
      linksCollection.forEach(el => el.classList.remove("open"));
...
function initMegaMenu(){
  ...
  const customMenu = panel.querySelector("[data-custom-category-menu]");
  if (customMenu && typeof rashwearCategories === "function") {
    const builtInCategories = new Set(["men", "women", "kids"]);
    customMenu.innerHTML = Object.entries(rashwearCategories()).filter(([key]) => !builtInCategories.has(key)).map(([key, category]) => `
      <h4><a href="/shop?cat=${encodeURIComponent(key)}">${category.label}</a></h4>
      <a href="/shop?cat=${encodeURIComponent(key)}">All ${category.label}</a>
      ${category.subs.map(sub => `<a href="/shop?cat=${encodeURIComponent(key)}&sub=${encodeURIComponent(sub)}">${sub}</a>`).join("")}
    `).join("");
    customMenu.hidden = !customMenu.innerHTML;
  }
...
function productCardHTML(p){
  const image = p.image ? `<img src="${p.image}" alt="${p.name}" loading="lazy">` : rashwearIcon(p.icon);
  ...
  return `
    <div class="product-card">
      <a href="/product?id=${p.id}" style="display:block;">
        <div class="product-media" style="${rashwearMediaStyle(p.swatch)}">
          <span class="price-tag">${fmtNPR(p.price)}</span>
          ${image}
          <button class="btn btn-primary btn-block quick-add" data-quick-add="${p.id}" ${soldOut ? "disabled" : ""} onclick="event.preventDefault(); quickAdd('${p.id}')">${soldOut ? "Out of stock" : "Quick add"}</button>
        </div>
      </a>
      <a href="/product?id=${p.id}">
        <div class="product-name">${p.name}</div>
        <div class="product-meta">
          <span class="product-cat">${rashwearCategoryLabel(p.cat)}</span>
          ${priceHTML}
        </div>
      </a>
    </div>`;
...
function quickAdd(productId){
  const p = rashwearCatalog().find(p => p.id === productId);
  if (!p) return;
...
function initHome(){
  ...
  if (!el) return;
  const products = rashwearCatalog();
  const featured = products.filter(p => p.tags && p.tags.includes("Bestseller")).concat(
    products.filter(p => !p.tags || !p.tags.includes("Bestseller"))
  ).slice(0, 8);
...
function initShop(){
  ...
  let sortMode = params.get("sort") || "default";
  const searchQ = (params.get("q") || "").trim().toLowerCase();
  const products = rashwearCatalog();
  const categories = rashwearCategories();
  const builtInCategories = new Set(["men", "women", "kids"]);
  if (filterBar) {
    Object.entries(categories).filter(([key]) => !builtInCategories.has(key)).forEach(([key, category]) => {
      const button = document.createElement("button");
      button.className = "filter-pill";
      button.dataset.filter = key;
      button.textContent = category.label;
      if (sortSelect) filterBar.insertBefore(button, sortSelect);
  ...
  function renderSubBar(){
    if (!subBar) return;
    const tree = rashwearCategories()[activeCat];
  ...
  if (sortSelect){
    ...
    sortSelect.addEventListener("change", () => {
      ...
      const url = new URL(location.href);
      if (sortMode === "default") url.searchParams.delete("sort"); else url.searchParams.set("sort", sortMode);
      history.replaceState(null, "", url);
      render();
    });
  }
...
function initProduct(){
  ...
  const params = new URLSearchParams(location.search);
  const products = rashwearCatalog();
  const p = products.find(p => p.id === params.get("id")) || products[0];
  ...
  wrap.querySelector("[data-pdp-media]").innerHTML = p.image ? `<img src="${p.image}" alt="${p.name}">` : rashwearIcon(p.icon);
  wrap.querySelector("[data-pdp-cat]").textContent = rashwearCategoryLabel(p.cat);
  ...
  if (Number.isFinite(Number(p.stock)) && Number(p.stock) <= 0) {
    wrap.querySelector("[data-add-cart]").disabled = true;
    wrap.querySelector("[data-add-cart]").textContent = "Out of stock";
  }
