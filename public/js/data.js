...
let RASHWEAR_DATABASE_CATALOG_LOADED = false;
...
const RASHWEAR_ADMIN_PRODUCTS_KEY = "rashwear_admin_products";
const RASHWEAR_ADMIN_CATEGORIES_KEY = "rashwear_admin_categories";

async function rashwearHydrateCatalog() {
  try {
    const response = await fetch("/api/catalog", { headers: { Accept: "application/json" } });
    if (!response.ok) return;
    const data = await response.json();
    RASHWEAR_DATABASE_CATALOG_LOADED = true;
    if (Array.isArray(data.categories) && data.categories.length) {
      RASHWEAR_CATEGORIES = data.categories.reduce((result, category) => {
        result[category.slug] = {
          label: category.name,
          subs: (data.subcategories || []).filter(subcategory => subcategory.category_id === category.id).map(subcategory => subcategory.name)
        };
        return result;
      }, {});
    }
    if (Array.isArray(data.products)) {
      RASHWEAR_PRODUCTS = data.products.map(product => ({
        ...product,
        cat: product.cat || "",
        sub: product.sub || "",
        ...
        icon: product.icon || "tshirt",
        swatch: product.swatch || "stone2",
        tags: product.tags || []
      }));
    }
  } catch (error) {
    // Static data remains available when the database API is offline.
  }
}

function rashwearCategories() {
  if (RASHWEAR_DATABASE_CATALOG_LOADED) return RASHWEAR_CATEGORIES;
  let saved = {};
  try {
    saved = JSON.parse(localStorage.getItem(RASHWEAR_ADMIN_CATEGORIES_KEY) || "{}");
  } catch (error) {
    saved = {};
  }
  const storedCategories = saved.categories || saved;
  const deleted = saved.deleted || [];
  const categories = Object.keys(storedCategories).reduce((result, key) => {
    const category = storedCategories[key];
    if (category && category.label && Array.isArray(category.subs) && category.subs.length) {
      result[key] = { label: category.label, subs: category.subs };
    }
    return result;
  }, { ...RASHWEAR_CATEGORIES });
  deleted.forEach(key => delete categories[key]);
  return categories;
}

function rashwearCategoryLabel(categoryKey) {
  const category = rashwearCategories()[categoryKey];
  return category ? category.label : categoryKey;
}

function rashwearCatalog() {
  if (RASHWEAR_DATABASE_CATALOG_LOADED) return RASHWEAR_PRODUCTS;
  let saved = [];
  try {
    saved = JSON.parse(localStorage.getItem(RASHWEAR_ADMIN_PRODUCTS_KEY) || "[]");
  } catch (error) {
    saved = [];
  }

  const managed = saved.filter(product => product.status !== "archived" && product.status !== "draft").map(product => ({
    ...product,
    cat: String(product.category || "").toLowerCase().replace(/\s+/g, "-"),
  ...
  return RASHWEAR_PRODUCTS.filter(product => !managedIds.has(product.id)).concat(managed);
}
